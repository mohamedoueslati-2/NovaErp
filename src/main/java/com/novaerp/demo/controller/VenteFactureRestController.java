...
@RestController
@RequestMapping("/api/sales/factures")
@PreAuthorize("hasRole('VENTE')")
@RequiredArgsConstructor
public class VenteFactureRestController {

    private final FactureService factureService;

    @GetMapping
    public ResponseEntity<?> listFactures(@ModelAttribute FactureSearchCriteria criteria,
    ...
    public ResponseEntity<?> facturesNonPayees() {
        return ResponseEntity.ok(factureService.findFacturesNonPayees(FactureType.FACTURE_CLIENT));
    }

    @GetMapping("/statistics")
    public ResponseEntity<?> statistics() {
        return ResponseEntity.ok(factureService.getFactureStatistics(FactureType.FACTURE_CLIENT));
    }
}
